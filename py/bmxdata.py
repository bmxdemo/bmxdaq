...
class BMXFile(object):
    ...
    def __init__(self, fname, nsamples=None, force_version=None, loadD2=False, loadRFI=False, verbose=0):
        ...
        if self.version >= 6:
            if verbose > 0:
                print("CardMask: %i, Channels: %i,  Cuts: %i" %
                      (self.cardMask, self.nChan, self.ncuts))
        else:
        ...
        for i in range(self.ncuts):
            if verbose > 0:
                print("    Cut ", i, " ",
                      self.numin[i], '-', self.numax[i], 'MHz #P=', self.nP[i])
            self.freq.append(self.freqOffset+self.numin[i]+(
                np.arange(self.nP[i])+0.5)*(self.numax[i]-self.numin[i])/self.nP[i])
        rec_desc = []
        self.haveMJD = False
    ...
    def joinD2(self, D2, loadRFI=False):
        ...
        while True:
            p0, pp, pm = mjdoffset(
                offset)**2, mjdoffset(offset+1)**2, mjdoffset(offset-1)**2
            if p0 < pp and p0 < pm:
                break
            elif pm < pp:
                offset -= 1
        ...
        for n in data1.dtype.names:
            if 'chan' in n:
                ...
                d2[n] = n.replace('1', '5').replace(
                    '2', '6').replace('3', '7').replace('4', '8')
        ...
        if loadRFI:
            ...
            rfimask1, rfimask2 = getSlices(
                offset, d1data=self.rfimask, d2data=D2.rfimask)
            rfinumbad1, rfinumbad2 = getSlices(
                offset, d1data=self.rfinumbad, d2data=D2.rfinumbad)
            rfid2 = {}
            for n in data1.dtype.names:
                if 'chan' in n:
                    rfid2[n] = n.replace('1', '5').replace(
                        '2', '6').replace('3', '7').replace('4', '8')
            ...
            self.rfinumbad = rf.merge_arrays(
                (rfinumbad1, rfinumbad2), flatten=True)
        if (self.verbose > 0):
    ...
    def getNames(self, cut):
        cutstr = "_"+str(cut)
        i = len(cutstr)
        chnames = sorted([n for n in self.names if (
            'chan' in n) and (n[-i:] == cutstr)])
        return chnames
    ...
    def getRadar(self, fmin=1242., fmax=1247.):
        ...
        da = (self.data[:]['chan1_0'])[:, imin:imax] + \
            (self.data[:]['chan2_0'])[:, imin:imax]
        da = da.mean(axis=1)
    ...
    def plotWaterfall(self, fmin=None, fmax=None, nsamples=None, cut=0, binSize=4, subtractMean=False, minmax=None):
        ...
        for n in range(2):
            ...
            for i in range(nsamples):
                arr.append(
                    self.data[i]['chan' + str(n+1)+'_' + str(cut)][imin:imax])
            ...
            if(subtractMean):
                ...
                if minmax is not None:
                    ...
                    vmin = None
                    vmax = None
                plt.imshow(arr, interpolation="nearest", vmin=vmin, vmax=vmax,
                           aspect="auto", extent=[fmin, fmax, nsamples*self.deltaT, 0])
            else:
                plt.imshow(arr, norm=colors.LogNorm(), interpolation="nearest",
                           aspect="auto", extent=[fmin, fmax, nsamples*self.deltaT, 0])
            plt.colorbar()
    ...
    def getImageArray(self, binSize=[1, 1], nsamples=None, cut=None):
        ...
        if binSize[1] > 1:
            ...
            for i in range(int(len(arr)/binSize[1])):
                reducedArr[n].append(
                    arr[binSize[1]*i:binSize[1]*(i+1)].mean(axis=0))
        else:
            reducedArr = arr
    ...
    def loadRFI(self, fname_rfi=None):
        ...
        if fname_rfi is None:
            fname = self.fname
            fname_rfi = os.path.join(os.path.dirname(
                fname), 'rfi', os.path.basename(fname).replace('data', 'rfi'))
        # if RFI file does not exist, exit with error
        if not os.path.isfile(fname_rfi):
...
class BMXRingbuffer (object):
    def __init__(self, fname, force_version=None):
        ...
        data = np.fromfile(f, np.dtype(
            [('ch1', 'i1'), ('ch2', 'i1')], align=False))
